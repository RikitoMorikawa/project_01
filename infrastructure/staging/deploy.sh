...
# CSR Lambda API System - Staging Environment Deployment Script
# ステージング環境のインフラストラクチャをデプロイするスクリプト
...
pre_deployment_checks() {
    ...
    DEV_VPC_ID=$(aws cloudformation describe-stacks \
        --stack-name "${PROJECT_NAME}-dev-network" \
        --region "$REGION" \
        --query 'Stacks[0].Outputs[?OutputKey==`VPCId`].OutputValue' \
        --output text 2>/dev/null || echo "")
    
    if [[ -n "$DEV_VPC_ID" ]]; then
        log_info "開発環境のVPCが検出されました: $DEV_VPC_ID"
        log_info "ステージング環境は異なるCIDRブロック（10.1.0.0/16）を使用します"
    fi
...
# メイン処理
main() {
    log_info "=== CSR Lambda API System - ステージング環境デプロイメント開始 ==="
    
    # 現在のディレクトリを確認
    if [[ ! -f "main.yaml" ]]; then
        log_error "main.yaml が見つかりません。infrastructure/staging ディレクトリで実行してください。"
        exit 1
    fi
    ...
    STAGING_PARAMS=$(load_parameters "parameters.json")
    
    # ステージング環境用のVPC CIDRを上書き
    STAGING_PARAMS=$(echo "$STAGING_PARAMS" | sed 's/ParameterKey=VpcCidr,ParameterValue=[^[:space:]]*/ParameterKey=VpcCidr,ParameterValue=10.1.0.0\/16/')
    STAGING_PARAMS=$(echo "$STAGING_PARAMS" | sed 's/ParameterKey=PublicSubnet1Cidr,ParameterValue=[^[:space:]]*/ParameterKey=PublicSubnet1Cidr,ParameterValue=10.1.1.0\/24/')
    STAGING_PARAMS=$(echo "$STAGING_PARAMS" | sed 's/ParameterKey=PublicSubnet2Cidr,ParameterValue=[^[:space:]]*/ParameterKey=PublicSubnet2Cidr,ParameterValue=10.1.2.0\/24/')
    STAGING_PARAMS=$(echo "$STAGING_PARAMS" | sed 's/ParameterKey=PrivateSubnet1Cidr,ParameterValue=[^[:space:]]*/ParameterKey=PrivateSubnet1Cidr,ParameterValue=10.1.3.0\/24/')
    STAGING_PARAMS=$(echo "$STAGING_PARAMS" | sed 's/ParameterKey=PrivateSubnet2Cidr,ParameterValue=[^[:space:]]*/ParameterKey=PrivateSubnet2Cidr,ParameterValue=10.1.4.0\/24/')
    
    ALL_PARAMS="$SHARED_PARAMS $STAGING_PARAMS"
...
def handler(event, context):
    ...
    response = {
        ...
        'body': json.dumps({
            'message': 'Hello from Staging Lambda! Please deploy the actual application code.',
            'environment': 'staging',
...
EOF
    ...
    # デプロイメント完了の確認
    log_success "=== ステージング環境のデプロイメントが完了しました ==="
    ...
    echo "5. API とフロントエンドの動作確認"
    echo "6. 監視アラームの設定確認"
    echo "7. セキュリティテストの実行"
    
    log_info "--- 監視とアラーム ---"
    echo "CloudWatch ダッシュボードでメトリクスを確認してください"
    echo "SNS トピックにアラーム通知の購読者を追加してください"
}
